...
class SelectConstructor {
	constructor(props, data = null) {
		...
		if (this.config.init) {
			...
			const selectItems = data
				...
				: document.querySelectorAll('select[data-fls-select]');
	...
	selectBuild(originalSelect) {
		...
		originalSelect.hasAttribute('data-fls-select-search')
	...
	selectAction(selectItem) {
		...
		const selectOpenzIndex = originalSelect.dataset.flsSelectZIndex
	...
	getSelectTitleValue(selectItem, originalSelect) {
		...
		if (
			originalSelect.multiple &&
			originalSelect.hasAttribute('data-fls-select-tags')
		) {
			selectTitleValue = this.getSelectedOptionsData(originalSelect)
				.elements.map(
					(option) =>
						`<span role="button" data-fls-select-id="${
							selectItem.dataset.flsSelectId
						}" data-fls-select-value="${
							option.value
			...
			if (
				originalSelect.dataset.flsSelectTags &&
				document.querySelector(originalSelect.dataset.flsSelectTags)
			) {
				document.querySelector(originalSelect.dataset.flsSelectTags).innerHTML =
					selectTitleValue;
		...
		if (!originalSelect.hasAttribute('data-fls-select-tags')) {
			selectTitleValue = selectTitleValue
				? selectTitleValue.map((item) => item.replace(/"/g, '&quot;'))
				: '';
		}
	...
	getOptions(originalSelect) {
		...
		const selectOptionsScroll = originalSelect.hasAttribute(
			...
			? ``
			: '';
		const customMaxHeightValue = +originalSelect.dataset.flsSelectScroll
			? +originalSelect.dataset.flsSelectScroll
			: null;
	...
	setOptionsPosition(selectItem) {
		...
		const customMaxHeightValue = +originalSelect.dataset.flsSelectScroll
			? `${+originalSelect.dataset.flsSelectScroll}px`
			: ``;
		const selectOptionsPosMargin = +originalSelect.dataset
			.flsSelectOptionsMargin
			? +originalSelect.dataset.flsSelectOptionsMargin
			: 10;
