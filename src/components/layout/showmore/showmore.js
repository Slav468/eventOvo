...
import { slideUp, slideDown, dataMediaQueries } from '@js/common/functions.js';
...
export function showMore() {
	...
	function getHeight(showMoreBlock, showMoreContent) {
		let hiddenHeight = 0;
		const showMoreType = showMoreBlock.dataset.flsShowmore
			? showMoreBlock.dataset.flsShowmore
			: 'size';
		const rowGap = parseFloat(getComputedStyle(showMoreContent).rowGap)
			? parseFloat(getComputedStyle(showMoreContent).rowGap)
			: 0;
		if (showMoreType === 'items') {
			const showMoreTypeValue = showMoreContent.dataset.flsShowmoreContent
	...
	function showMoreActions(e) {
		...
		if (targetType === 'click') {
			if (targetEvent.closest('[data-fls-showmore-button]')) {
				...
				const showMoreSpeed = showMoreBlock.dataset.flsShowmoreButton
					? showMoreBlock.dataset.flsShowmoreButton
					: '500';
				const hiddenHeight = getHeight(showMoreBlock, showMoreContent);
